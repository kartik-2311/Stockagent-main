"""
Utility configuration. API keys are read from environment variables by default.
You can either set the `OPENAI_API_KEY` and `GOOGLE_API_KEY` environment variables
or put the values directly below (not recommended for security reasons).
"""
import os

# Prefer environment variables; fall back to hard-coded values if you set them here.
OPENAI_API_KEY = os.environ.get('OPENAI_API_KEY', '')
GOOGLE_API_KEY = os.environ.get('GOOGLE_API_KEY', '')

# 基础设置
AGENTS_NUM = 50  # agent数量
TOTAL_DATE = 264   #Number of traders
TOTAL_SESSION = 3   # Daily number of trades
...
# 财报
SEASONAL_DAYS = 66 # 一quarterly time
SEASON_REPORT_DAYS = [12, 78, 144, 210] # Earnings Release Date
...
EVENT_1_LOAN_RATE = [0.024, 0.027, 0.030] # The interest rate after the reserve requirement cut is placed here
