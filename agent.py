...
class Agent:
    ...
    def run_api(self, prompt, temperature: float = 1):
        if 'mock' in self.model:
            return self.run_api_mock(prompt, temperature)
        elif 'gpt' in self.model:
            return self.run_api_gpt(prompt, temperature)
        elif 'gemini' in self.model:
            return self.run_api_gemini(prompt, temperature)

    def run_api_mock(self, prompt, temperature: float = 1):
        """Mock LLM for testing without API calls. Returns realistic JSON responses."""
        # Add to chat history for consistency
        self.chat_history.append({"role": "user", "content": prompt})
        
        # Determine response based on prompt content (simple heuristics)
        if "loan" in prompt.lower():
            # Mock loan decision: ~50% yes
            response = '{"loan": "%s", "amount": %.2f, "loan_type": %d, "repayment_date": %d}' % (
            ...
            # Mock stock action: buy, hold, or sell with random quantities
            action = random.choice(["buy", "hold", "sell"])
            response = '{"action": "%s", "stock": "%s", "amount": %d}' % (
                action,
                random.choice(["A", "B"]),
                random.randint(0, 100) if action != "hold" else 0
            )
        else:
            # Default mock response
            response = '{"status": "ok"}'
        
        # Add mock response to chat history for consistency
        self.chat_history.append({"role": "assistant", "content": response})
        log.logger.info("MOCK LLM response: %s" % response)
        return response
